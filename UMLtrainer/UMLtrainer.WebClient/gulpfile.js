...
var bundle = function (changedFiles) {
    ...
    var builderStream = builder
        .bundle()
        .on('error', plugins.util.log)
        .pipe(source('app.js'))
        .pipe(gulp.dest(outputApp));

    if (changedFiles) {
        var lintStream = gulp.src(changedFiles)
            .pipe(plugins.tslint(tsFormatterConfig))
            .pipe(plugins.tslint.report(tsReporterConfig));
...
gulp.task("ts:lint", function() {
    return gulp.src(inputApp + extensionTs)
        .pipe(plugins.tslint(tsFormatterConfig))
        .pipe(plugins.tslint.report(tsReporterConfig));
});
...
gulp.task("inject", function () {   
    return gulp
        .src(inputHtml + inpuptHtmlName)
        // css
        .pipe(plugins.inject(
            gulp.src(customCssOutput),
            {
                addRootSlash: false,
